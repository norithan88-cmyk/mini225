#!/usr/bin/env python3
"""
AI ミニ225研究所 - 本日のAIシグナル 自動計算スクリプト
...
やっていること（概要）:
  ...
  ★なぜNIY=Fか: 実際の大阪取引所ミニ225先物には無料で使えるAPIが
    見つからなかった。NIY=Fは同じ日経225指数を原資産とする円建て先物で、
    CME Globexでほぼ24時間取引されており、日中しか動かない現物指数
    （^N225）よりミニ225の値動きの性質に近い代理指標として採用している。
    ミニ225そのものとは別市場である点に注意。
...
def build_signal(out_path=None):
    ...
    result = {
        ...
        "pair": "日経225先物（NIY=F、CME円建て、ミニ225の代理指標）",
        "latest_price": round(latest_price, 1),
        "variants": variants_result,
        "disclaimer": (
            "本データはルールベースの参考情報であり、投資成果を保証するものではありません。"
            "実際の大阪取引所ミニ225先物ではなく、同じ日経225指数を原資産とする"
